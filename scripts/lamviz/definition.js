...
function getNextVariableName() {
  ...
  for (
    var i = 0;
    i < Math.floor(currentVariableIndex / variableNames.length);
    i++
  ) {
    name += "'";
  }
...
function getNextFreeVariableName() {
  ...
  for (
    var i = 0;
    i < Math.floor(currentFreeVariableIndex / freeVariableNames.length);
    i++
  ) {
    name += "'";
  }
...
class LambdaAbstraction {
  ...
  prettyPrintLabels(ctx, x) {
    ...
    var string =
      "\u03BB" + this.label + ". " + this.t.prettyPrintLabels(ctx, 0);
...
class LambdaApplication {
  ...
  prettyPrint(x) {
    ...
    if (x === 0) {
      ...
      string =
        "(" + this.t1.prettyPrint(x) + " " + this.t2.prettyPrint(x + 1) + ")";
    }
  ...
  prettyPrintLabels(ctx, x) {
    ...
    if (x === 0) {
      if (this.t1.getType() === ABS && this.t1.alias === "") {
        string =
          "(" +
          this.t1.prettyPrintLabels(ctx, 0) +
          ") " +
          this.t2.prettyPrintLabels(ctx, 1);
      } else {
        string =
          this.t1.prettyPrintLabels(ctx, 0) +
          " " +
      ...
      string =
        "(" +
        this.t1.prettyPrintLabels(ctx, x) +
        " " +
        this.t2.prettyPrintLabels(ctx, x + 1) +
        ")";
    }
  ...
  hasBetaRedex() {
    return (
      this.isBetaRedex() || this.t1.hasBetaRedex() || this.t2.hasBetaRedex()
    );
  }
  ...
  printRedexes(ctx) {
    ...
    return array.concat(
      this.t1.printRedexes(ctx).concat(this.t2.printRedexes(ctx)),
    );
  }
  ...
  printGraph() {
    ...
    for (var i = 0; i < this.matrix.length; i++) {
      ...
      for (var j = 0; j < this.matrix[i][1].length; j++) {
        string +=
          "\n----" +
          this.matrix[i][1][j][0].prettyPrint() +
          " via " +
          this.matrix[i][1][j][1].prettyPrint();
      }
