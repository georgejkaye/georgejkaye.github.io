...
function generateMapElements(
  ...
  if (array === undefined) {
    ...
    for (var i = 0; i < ctx.length(); i++) {
      ...
      const lambdaAbstractionSupportNodeID = checkID(
        newFreeVariableID + "._abstraction_node_right",
        nodes,
      );
      array = defineNode(array, lambdaAbstractionSupportNodeID, varNode, []);

      const lambdaAbstractionSupportEdgeID = checkID(
        newFreeVariableID + "._node_from_abstraction_to_right",
        edges,
      );
      array = defineEdge(
        array,
        lambdaAbstractionSupportEdgeID,
        varEdge,
        [],
        newFreeVariableID,
        lambdaAbstractionSupportNodeID,
      );

      /* The abstracted variable travels to the top of the map */
      const lambdaAbstractionSupportNode2ID = checkID(
        newFreeVariableID + "._abstraction_node_top",
        nodes,
      );
      array = defineNode(
        array,
        lambdaAbstractionSupportNode2ID,
        varNodeTop,
        [],
      );

      const lambdaAbstractionSupportEdge2ID = checkID(
        newFreeVariableID + "._edge_from_abstraction_to_top",
        edges,
      );
      array = defineEdge(
        array,
        lambdaAbstractionSupportEdge2ID,
        varEdge,
        [],
        lambdaAbstractionSupportNodeID,
        lambdaAbstractionSupportNode2ID,
  ...
  switch (term.getType()) {
    case ABS:
      ...
      newEdgeID = checkID(
        newNodeID + " " + term.t.prettyPrintLabels(ctx),
        edges,
      );
      newEdgeType = absEdge;
      ...
      const lambdaAbstractionSupportNodeID = checkID(
        newNodeID + "._abstraction_node_right",
        nodes,
      );
      array = defineNode(
        array,
      ...
      const abstractionMidpointID =
        newNodeID + "_midpoint_" + lambdaAbstractionSupportNodeID;
      array = defineNode(
        array,
        abstractionMidpointID,
        absVarMidpoint,
        classes,
        0,
        0,
      ...
      const lambdaAbstractionSupportEdgeID = checkID(
        newNodeID + "._node_from_abstraction_to_right",
        edges,
      );
      array = defineEdge(
        array,
        lambdaAbstractionSupportEdgeID,
        varEdge,
        classes.concat("no-arrows"),
        newNodeID,
        abstractionMidpointID,
      );

      const lambdaAbstractionSupportEdgeMidpointID = checkID(
        newNodeID + "._node_from_abstraction_to_right_midpoint",
        edges,
      );
      array = defineEdge(
        array,
        lambdaAbstractionSupportEdgeMidpointID,
        varEdge,
        classes,
        abstractionMidpointID,
        lambdaAbstractionSupportNodeID,
      ...
      /* The abstracted variable travels to the top of the map */
      const lambdaAbstractionSupportNode2ID = checkID(
        newNodeID + "._abstraction_node_top",
        nodes,
      );
      array = defineNode(
        array,
      ...
      const lambdaAbstractionSupportEdge2ID = checkID(
        newNodeID + "._edge_from_abstraction_to_top",
        edges,
      );
      array = defineEdge(
        array,
      ...
      var scopeArray = generateMapElements(
        term.t,
        ctx,
        [],
        newNodeID,
      ...
      if (rightmostScope >= posX) {
        scopeArray = shiftXs(
          scopeArray,
          -(rightmostScope - posX) - nodeDistanceX,
        );
      }
    ...
    case APP:
      ...
      newNodeID = checkID(
        "[" +
          term.t1.prettyPrintLabels(ctx) +
          " @ " +
          term.t2.prettyPrintLabels(ctx) +
          "]",
        nodes,
      );
      ...
      var lhsArray = generateMapElements(
        term.t1,
        ctx,
        [],
        newNodeID,
      ...
      var rhsArray = generateMapElements(
        term.t2,
        ctx,
        [],
        newNodeID,
    ...
    case VAR:
      ...
      newEdgeID = checkID(
        variableID + " in " + parent + "_edge_from_parent_to_variable",
        edges,
      );
      newEdgeType = varEdge;
      ...
      const lambdaVariableSupportNodeID = checkID(
        variableID + "_variable_node_top",
        nodes,
      );
      array = defineNode(
        array,
        lambdaVariableSupportNodeID,
        varNodeTop,
        classes,
        posX,
        posY - nodeDistanceY,
      ...
      const lambdaVariableSupportEdgeID = checkID(
        variableID + " in " + parent + "_edge_from_top_to_variable",
        edges,
      );
      array = defineEdge(
        array,
        lambdaVariableSupportEdgeID,
        varEdge,
        classes,
        lambdaVariableSupportNodeID,
        newNodeID,
      ...
      var lambdaAbstractionNodeID =
        lambda + variableID + "._abstraction_node_top";

      /* Create the edge connecting the node at the top to the corresponsing abstraction support node */
      const lambdaVariableAbstractionEdgeID = checkID(
        variableID +
          " in " +
          parent +
          "_curved_edge_from_abstraction_to_variable",
        edges,
      );
      array = defineEdge(
        array,
        lambdaVariableAbstractionEdgeID,
        varEdgeLabel,
        classes,
        lambdaAbstractionNodeID,
        lambdaVariableSupportNodeID,
        variableLabel,
      );
...
function createNode(id, type, classes, x, y, label, level, free) {
  ...
  return {
    group: "nodes",
    data: { id: id, type: type, label: label, level: level, free: free },
    position: { x: x, y: y },
    classes: classes,
  };
}
...
function createEdge(id, type, classes, source, target, label) {
  ...
  return {
    group: "edges",
    data: { id: id, source: source, target: target, type: type, label: label },
    classes: classes,
  };
}
...
function furthestLeft(array) {
  ...
  for (i = 1; i < array.length; i++) {
    if (
      array[i].position !== undefined &&
      !array[i].data.type.includes("midpoint")
    ) {
...
function placeFreeVariables(boundVariables, freeVariables, ctx) {
  ...
  for (j = freeVariables.length - 1; j >= 0; j--) {
    ...
    var x =
      mapRightest +
      (freeVariables.length - j) * nodeDistanceX * 2 -
      nodeDistanceX;
    ...
    cyMap
      .elements("[id = '" + id + "._abstraction_node_right']")
      .position("x", x + nodeDistanceX);
    cyMap
      .elements("[id = '" + id + "._abstraction_node_right']")
      .position("y", -nodeDistanceY);
    cyMap
      .elements("[id = '" + id + "._abstraction_node_top']")
      .position("x", x + nodeDistanceX);
  }
}
...
function drawMap(id, term, ctx, zoom, pan, labels) {
  ...
  cyMap = cytoscape({
    ...
    style: [
      ...
      {
        selector:
          'node[type ="' + varNode + '"], node[type ="' + varNodeTop + '"]',
        style: {
          width: "5",
          height: "5",
...

      {
        ...
        style: {
          ...
          "font-size": "6",
          "transition-property":
            "background-color, line-color, mid-target-arrow-color",
          "transition-duration": "0.25s",
          opacity: "0s",
        },
      },
  ...
  placeFreeVariables(
    cyMap.elements(getNodeTypeText(varNodeTop)),
    cyMap.elements(getNodeTypeText(absNodeFree)),
    ctx,
  );

  /** Pull all the variable arcs to the top of the map so crossings are clear. */
  cyMap
    .elements(getNodeTypeText(varNodeTop))
    .position("y", highest - nodeDistanceY / 2);
  ...
  for (var i = 0; i < redexList.length; i++) {
    ...
    if (abstractionNodeTop.data("free") !== "true") {
      var edgeFromAbstractionToTop = cyMap.elements(
        'edge[target="' + redexList[i][0] + '"]',
      );
      var abstractionNodeRight = edgeFromAbstractionToTop.source();
      var supportNodeToMidpoint = cyMap.elements(
        'edge[target="' + abstractionNodeRight.id() + '"]',
      );
      var midpoint = supportNodeToMidpoint.source();
      var abstractionNodeToMidpoint = cyMap.elements(
        'edge[target="' + midpoint.id() + '"]',
      );
  ...
  cyMap.fit(
    cyMap.filter(function (ele, i, eles) {
      return true;
    }),
    10,
  );
...
function performReductionAnimation(i) {
  ...
  var appMids = appEles
    .connectedNodes()
    .filter((x) => x.data().id.includes("midpoint"));
  ...
  var absVarSupportNode = cyMap
    .edges('[source = "' + absvarid + '"]')[0]
    .target();
  ...
  var RHSMidpointX =
    (absVarSupportNode.position("x") + argSupportNode.position("x")) / 2;
  var RHSMidpointY =
    (absVarSupportNode.position("y") + argSupportNode.position("y")) / 2;

  /* Join together the remaining midpoints */
  absvar.animate(
    { position: { x: RHSMidpointX, y: RHSMidpointY } },
    { duration: 1000 },
  );
  arg.animate(
    { position: { x: RHSMidpointX, y: RHSMidpointY } },
  ...
  body.animate(
    { position: { x: LHSMidpointX, y: LHSMidpointY } },
    { duration: 1000 },
  );
  term.animate(
    { position: { x: LHSMidpointX, y: LHSMidpointY }, style: { opacity: 0 } },
    { duration: 1000 },
  );
}
...
function generateNormalisationGraphElements(id, graph, ctx, maps) {
  ...
  for (var i = 0; i < graph.matrix.length; i++) {
    ...
    array = defineNode(
      ...
      "",
      0,
      level * normalisationDistanceY,
      term.prettyPrintLabels(ctx),
      level,
      "",
    );
    ...
    for (var j = 0; j < reductions.length; j++) {
      array = defineEdge(
        ...
        nodeID +
          "-b->" +
          reductions[j][1].prettyPrint() +
          "-b->" +
          reductions[j][0].prettyPrint(),
        "norm",
        "",
        nodeID,
        reductions[j][0].prettyPrint(),
...
function drawNormalisationGraph(id, term, ctx, maps, labels, arrows) {
  ...
  for (var i = 0; i < elems.length; i++) {
    if (
      elems[i].group !== "edges" ||
      normNodes.includes(elems[i].data.target)
    ) {
  ...
  /* Fit the map to the frame. */
  cyNorm.fit(
    cyNorm.filter(function (ele, i, eles) {
      return true;
    }),
    10,
  );
