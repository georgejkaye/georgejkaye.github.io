...
function getStats(currentTerm, labels) {
  ...
  return (
    getRow(
      getCell(
        "term-heading",
        '<b id = "term-name">' + printTermHTML(currentTerm, false) + "</b>",
      ),
    ) +
    getRow(
      getCell(
        "term-subheading",
        '<b id = "term-name-bruijn">' +
          printTermHTML(currentTerm, true) +
          "</b>",
      ),
    ) +
    ...
    getRow(
      getCell("term-fact", "Abstractions: " + currentTerm.abstractions()),
    ) +
    getRow(
      getCell("term-fact", "Applications: " + currentTerm.applications()),
    ) +
    getRow(
      getCell("term-fact", "Total variables: " + currentTerm.variables()),
    ) +
    getRow(
      getCell(
        "term-fact",
        "Unique variables: " + currentTerm.uniqueVariables(),
      ),
    ) +
    getRow(
      getCell("term-fact", "Free variables: " + currentTerm.freeVariables()),
    ) +
    ...
    getRow(
      getCell(
        "term-fact",
        bulletsOfArray(
          currentTerm.printRedexes(freeVariables),
          "redex",
          "clickRedexOnclick(i,)",
          "highlightRedexMouseover(i, true)",
          "unhighlightRedexMouseover(i, true)",
        ),
      ),
    ) +
    getRow(
    ...
    getRow(
      getCell(
        "",
        'Draw maps (very costly for large maps) <input type = "checkbox" id = "normalisation-maps" checked>',
      ),
    ) +
    getRow(
      getCell(
        "",
        'Draw arrows (very costly for large maps) <input type = "checkbox" id = "normalisation-arrows" checked>',
      ),
    ) +
    getRow(
      getCell(
        "",
        'Draw labels (can get cluttered for large maps) <input type = "checkbox" id = "normalisation-labels" checked>',
      ),
    ) +
    getRow(
      getCell(
  ...
  if (!bigScreen) {
    changeText(
      ...
      "<table>" +
        ...
        "<td>" +
        getDiv(
          "w3-container frame big-frame",
          "frame" + currentFrame,
          "",
          "",
          getDiv("w3-container portrait", "portrait" + i, "", "", ""),
        ) +
        "</td>" +
        "<td>" +
        "<table>" +
    ...
    changeText(
      exhibit,
      getDiv(
        "w3-container frame full-frame",
        "frame" + currentFrame,
        "",
        "",
        getDiv("w3-container portrait", "portrait" + currentFrame, "", "", ""),
      ) +
        "<br>" +
        getSpan(
          "term-heading",
          ...
          "",
          "",
          "<b>" + printTermHTML(currentTerm, false) + "</b>&ensp;",
        ) +
        getSpan(
          "term-subheading",
          "",
          "",
          "",
          "<b>" + printTermHTML(currentTerm, true) + "</b>",
        ) +
        getDiv(
          "",
          "",
          ...
          getButton(
            "fullScreen-btn",
            "exitFullScreenMapButton('" + exhibit + "');",
            "Exit full screen",
          ) +
            getButton(
              "watch-reduction-btn",
...
function showNormalisationGraph() {
  ...
  changeText(
    "normalisation-studio",
    "<table>" +
      "<tr>" +
      "<td>" +
      getDiv(
        "w3-container frame graph-frame",
        "normalisation-graph-frame",
        "",
        "",
        getDiv("w3-container portrait", "normalisation-graph", "", "", ""),
      ) +
      "</td>" +
  ...
  var norm = drawNormalisationGraph(
    "normalisation-graph",
    currentTerm,
    freeVariables,
    document.getElementById("normalisation-maps").checked,
    document.getElementById("normalisation-labels").checked,
    document.getElementById("normalisation-arrows").checked,
  );
  ...
  if (pathStats) {
    ...
    pathStatsText =
      getRow(
        getCell(
          "term-fact",
          "Total paths: " + currentReductions.totalPathsToNormalForm(),
        ),
      ) +
      getRow(
        getCell(
          "term-fact",
          "Shortest path: " + currentReductions.shortestPathToNormalForm(),
        ),
      ) +
      getRow(
        getCell(
          "term-fact",
          "Longest path: " + currentReductions.longestPathToNormalForm(),
        ),
      ) +
      ...
      getRow(
        getCell(
          "term-fact",
          "Median path: " + currentReductions.medianPathToNormalForm(),
        ),
      ) +
      getRow(
        getCell(
          "term-fact",
          "Mode path: " + currentReductions.modePathToNormalForm(),
        ),
      );
  } else {
    pathStatsText =
      ...
      getRow(
        getCell(
          "",
          "Generating the path stats can take a very long time for large graphs!",
        ),
      );
  }
  ...
  return (
    ...
    getRow(
      getCell(
        ...
        getButton(
          "fullscreen-norm-btn",
          "fullScreenNormalisationGraphButton()",
          "Full screen",
          false,
        ),
      ),
    ...
    getRow(
      getCell(
        "",
        getButton("clear-norm-btn", "clearNormalisationGraph()", "Back", false),
      ),
    ) +
    getRow(
      getCell(
  ...
  changeText(
    ...
    getDiv(
      ...
      getDiv(
        ...
        getButton(
          "fullScreen-btn",
          "exitFullScreenNormalisationGraphButton();",
          "Exit full screen",
        ),
      ),
  );
