  ...
  if (writeTerms) {
    for (var i = 0; i < sortedCategories.length; i++) {
      ...
      for (var j = 0; j < sortedCategories[i][1].length; j++) {
        ...
        var subcaptionSpan = getSpan(
          "subcaption",
          "portrait-subcaption-" + portraitCount,
          "",
          "",
          subcaption,
        );
        ...
        var caption = getSpan(
          "caption",
          "portrait-caption-" + portraitCount,
          "font-size:" + size + "%",
          "",
          termName,
        );
        ...
        if (drawTerms) {
          termString += getDiv(
            ...
            "frame" + portraitCount,
            "",
            "inGallery = false; viewPortrait('church-room', sortedCategories[" +
              i +
              "][1][" +
              j +
              "], false);",
            getDiv(
              "w3-container inner-frame",
              "",
              "",
              "",
              getDiv(
                "w3-container portrait",
  ...
  if (totalNumber != 0) {
    ...
    changeText(
      ...
      "Click on a term to learn more about it. " +
        getButton("clear-btn", "clearButton(true)", "Clear all", false),
    );
  }

  changeText(
    "number-of-terms",
    numString + " match the filtering criteria: " + percentage.toFixed(2) + "%",
  );
...
function sortTerms(filter) {
  ...
  if (mode !== DEFAULT) {
    ...
    var propertyName =
      currentProperty === 1 ? propertyNameSingle : propertyNamePlural;
    ...
    for (var i = 1; i < terms.length; i++) {
      ...
      if (currentProperty !== newProperty) {
        c++;
        propertyName =
          newProperty === 1 ? propertyNameSingle : propertyNamePlural;
        sortedCategories[c] = [
          propertyFunction(terms[i]) + " " + propertyName,
          [terms[i]],
        ];
...
function quickSortTerms(termList, propertyFunction, order) {
  ...
  var highers = quickSortTerms(
    termList.filter((x) => propertyFunction(x) >= propertyFunction(pivot)),
    propertyFunction,
    order,
  );
  var lowers = quickSortTerms(
    termList.filter((x) => propertyFunction(x) < propertyFunction(pivot)),
    propertyFunction,
    order,
  );
